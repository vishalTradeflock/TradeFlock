-- TradeFlock USA — seed data
-- Run after schema.sql. 15 business-desk stories across Tech, Markets, Leadership, Finance.

begin;

insert into public.categories (id, name, slug, description) values
  ($$11111111-1111-1111-1111-111111111111$$, $$Tech$$, $$tech$$, $$Silicon Valley, semiconductors, cloud, and the platforms that run the economy.$$),
  ($$22222222-2222-2222-2222-222222222222$$, $$Markets$$, $$markets$$, $$Equities, rates, commodities, and the tape that moves boardrooms.$$),
  ($$33333333-3333-3333-3333-333333333333$$, $$Leadership$$, $$leadership$$, $$CEOs, boards, labor, and the people who set corporate direction.$$),
  ($$44444444-4444-4444-4444-444444444444$$, $$Finance$$, $$finance$$, $$Banking, private credit, regulation, and the plumbing of American capital.$$)
on conflict (id) do update set
  name = excluded.name,
  slug = excluded.slug,
  description = excluded.description;

insert into public.authors (id, name, slug, bio, title, avatar_url) values
  ($$aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa1$$, $$Elena Vasquez$$, $$elena-vasquez$$, $$Covers Treasuries, the dollar, and the policy signals that reprice risk assets.$$, $$Senior Markets Correspondent$$, null),
  ($$aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa2$$, $$James Whitaker$$, $$james-whitaker$$, $$Reports on semiconductors, cloud infrastructure, and the industrial politics of AI.$$, $$Technology Editor$$, null),
  ($$aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa3$$, $$Priya Nair$$, $$priya-nair$$, $$Writes on banks, private credit, and the regulatory perimeter around nonbank lenders.$$, $$Finance Reporter$$, null),
  ($$aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa4$$, $$Marcus Chen$$, $$marcus-chen$$, $$Follows Fortune 500 succession, boards, and the labor bargains reshaping corporate America.$$, $$Leadership & Policy Correspondent$$, null),
  ($$aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa5$$, $$Sophia Brennan$$, $$sophia-brennan$$, $$Covers IPOs, buybacks, and the capital-markets calendar out of New York.$$, $$Wall Street Correspondent$$, null)
on conflict (id) do update set
  name = excluded.name,
  slug = excluded.slug,
  bio = excluded.bio,
...
insert into public.articles (
  id, slug, title, dek, excerpt, body, cover_image_url, cover_image_alt,
  category_id, author_id, is_featured, is_breaking, view_count, published_at
) values
  (
    $$00000000-0000-0000-0000-000000000001$$,
    $$apple-on-device-ai-suppliers-recalibrate$$,
    $$Apple's on-device AI push forces a recalibration across Silicon Valley suppliers$$,
    $$A quieter Siri overhaul is becoming a supply-chain story — and a margin story — for the firms that feed Cupertino.$$,
    ...
    $$<p>CUPERTINO, Calif. — Apple's latest on-device intelligence stack is less a consumer spectacle than a procurement memo. Memory makers, foundry partners, and the mid-tier firms that assemble camera and sensor modules are being asked to ship parts that run hotter, store more, and fail less — without the cover of a single, easily marketed "AI iPhone" supercycle.</p>
...
<p>"This is not a software overlay," said a veteran hardware analyst in San Francisco. "It is a tax on the physical iPhone, paid in silicon and thermals. The companies that collect that tax will show it in gross margin before Apple ever puts a new Siri demo on a keynote stage."</p>$$,
  ...
  (
    ...
    $$microsoft-azure-backlog-multi-year-cloud-contracts$$,
    $$Microsoft's Azure backlog hits a record as enterprises lock in multi-year cloud contracts$$,
    $$CIOs are trading flexibility for capacity — a tell that the AI buildout is no longer optional capex.$$,
    ...
    $$<p>REDMOND, Wash. — Microsoft's cloud business is starting to look less like a utility and more like a shipyard with a waitlist. People familiar with the company's enterprise pipeline said Azure's remaining performance obligations climbed again in the quarter, driven by multi-year deals that bundle ordinary software seats with scarce GPU capacity.</p>
...
<p>Investors should watch two numbers that will not show up in a keynote: how much of the backlog is GPU-tied versus general-purpose compute, and how often customers are allowed to re-slide those commitments if model architectures change. The first tells you about scarcity. The second tells you about lock-in.</p>$$,
    ...
    $$Earth viewed from orbit at night with city lights$$,
    $$11111111-1111-1111-1111-111111111111$$,
    $$aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa2$$,
    false,
    false,
    12680,
    $$2026-09-09T18:40:00.000Z$$
  ),
  (
    $$00000000-0000-0000-0000-000000000003$$,
    $$nvidia-custom-silicon-hyperscalers-gpu-narrative$$,
    ...
    $$<p>SANTA CLARA, Calif. — For two years the market treated Nvidia as a toll booth on the only road that mattered. That story is getting a footnote. Custom accelerators designed inside Amazon, Google, and Microsoft are moving from science projects to procurement line items, not as replacements for Blackwell-class GPUs but as a second source for inference and a bargaining chip in every supply meeting.</p>
...
<p>The near-term risk to Nvidia is not lost revenue. It is multiple compression if investors decide the monopoly is a phase rather than a permanent feature of the industry. The company's networking and software moat is real. So is the incentive, inside every hyperscaler, to make sure it is not the only moat that exists.</p>$$,
    ...
    true,
    15310,
    $$2026-09-09T14:05:00.000Z$$
  ),
  (
    $$00000000-0000-0000-0000-000000000004$$,
    $$ftc-app-store-remedy-platform-fees$$,
    $$The FTC's app store remedy begins to bite — and Wall Street is recalculating platform fees$$,
    $$A long-running fight over in-app payments is turning into a line-item risk for Apple and Google's services margins.$$,
...
<p>A New York media analyst put the sensitivity simply: every 100 basis points of take-rate compression is a services-margin event, not a hardware event. That is why the stock reaction has been uneven. Hardware cycles can be narrated. Fee erosion is a slow leak, and leaks are how quality businesses become merely good ones.</p>$$,
    ...
    $$People working with laptops in a modern office$$,
    $$11111111-1111-1111-1111-111111111111$$,
    $$aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa2$$,
    false,
    false,
    7420,
    $$2026-09-08T16:20:00.000Z$$
  ),
  (
...
<p>Watch the 2s10s curve and investment-grade issuance calendars into October. If corporates rush the window, they believe this backup is a gift. If they wait, they believe it is a trend.</p>$$,
  ...
  (
    ...
    $$oil-holds-82-opec-china-demand$$,
    $$Oil holds near $82 as OPEC+ discipline collides with soft China demand$$,
    $$Producers are defending the floor. The ceiling, for now, is a Chinese factory park running below last year's pulse.$$,
    ...
    $$<p>HOUSTON — Crude has become a market of small numbers. Brent hovering near $82 a barrel is not a crisis and not a boom. It is a standoff. OPEC+ has shown it can still withhold barrels. China has shown it can still disappoint the demand forecasts that oil desks recycle each spring.</p>
...
<p>Energy equities have noticed. The sector is no longer priced as a relic, nor as a moonshot. It is priced as a cash business with a geopolitical option attached — which, for a U.S. business desk, is the honest description.</p>$$,
  ...
  (
    ...
    $$dollar-rally-squeezes-emerging-market-borrowers$$,
    $$The dollar's quiet rally is squeezing emerging-market borrowers again$$,
    ...
    $$<p>NEW YORK — The dollar does not need a crisis to cause one elsewhere. A grinding rally, fed by relative U.S. growth and the slower Fed path now in the price, is lifting the local-currency cost of dollar bills for companies and governments that borrowed when money was cheaper and the greenback was less loved.</p>
...
<p>Investors should treat the split as a signal, not a solution. A weak independent chair is expensive stationery. A strong one is the cheapest risk control a board can buy.</p>$$,
    ...
    false,
    11270,
    $$2026-09-06T17:45:00.000Z$$
  ),
  (
    $$00000000-0000-0000-0000-000000000010$$,
    $$talent-war-ai-safety-officers$$,
    $$Inside the talent war for AI safety officers, a job that barely existed two years ago$$,
    $$Companies are hiring people to tell them what not to ship — and paying as if that were a revenue role.$$,
    ...
    $$<p>SAN FRANCISCO — Two years ago, "AI safety officer" was a punch line or a conference badge. This autumn it is a line in the budget. Large U.S. companies — not only model labs — are hiring officials whose job is to slow a product, document a risk, and take the call when a customer or a regulator asks who is in charge. Compensation, recruiters say, now rivals product leadership in a handful of searches.</p>
...
<p>Whether the title becomes a profession or a fad will be decided by the first serious incident that a company survives — or does not. Until then, the labor market is doing what it always does at the birth of a function: overpaying for scarce people and hoping the org chart catches up.</p>$$,
    ...
    $$Team discussion in a glass-walled office$$,
    $$33333333-3333-3333-3333-333333333333$$,
    $$aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa4$$,
    false,
    false,
  ...
  (
    ...
    $$starbucks-new-chief-labor-smaller-stores$$,
    $$Starbucks' new chief bets on labor peace and a smaller, faster store footprint$$,
    $$The coffee giant is trading square footage for throughput — and trying to end a union fight that became a brand problem.$$,
...
<p>Rivals at the value end and the third-wave end will keep nibbling. Starbucks still has the most valuable morning real estate in American retail. The question is whether it can run that real estate like a network instead of a collection of stages.</p>$$,
  ...
  (
    ...
    $$Private credit's $1.7 trillion moment: regulators want stress tests, lenders want time$$,
    $$A market that grew up in the cracks of banking is being asked to behave like one — without the deposit base or the public tape.$$,
...
<p>For corporate borrowers, the near-term effect of a heavier hand would be slower origination and slightly wider spreads — a nuisance, not a freeze. The longer-term effect is political. If private credit wants to remain the spare tire of American finance, it will have to accept that spare tires get inspected after they become the wheels.</p>$$,
    ...
    $$Calculator, charts, and financial documents on a desk$$,
  ...
  (
    ...
    $$regional-banks-cre-capital-rebuild$$,
...
<p>Washington's mood is watchful rather than panicked. Supervisors want higher capital on the books that deserve it and fewer surprises in shared-national-credit reviews. They are not, for the moment, writing obituaries. That is progress. It is not a green light.</p>$$,
    ...
    false,
    13220,
    $$2026-09-08T09:35:00.000Z$$
  ),
  (
    $$00000000-0000-0000-0000-000000000014$$,
    $$irs-direct-file-tax-prep-oligopoly$$,
...
<p>For investors, Direct File is not an extinction event. It is a ceiling on take-rate and a reminder that some American "markets" exist because the public option was not allowed to exist. When that condition changes, multiples should too.</p>$$,
    ...
    false,
    5790,
    $$2026-09-05T15:50:00.000Z$$
  ),
  (
    $$00000000-0000-0000-0000-000000000015$$,
    $$buyback-boom-cfos-2027-capex$$,
    ...
    $$<p>NEW YORK — Share repurchases are back in fashion, which is another way of saying corporate America is still more confident in its own stock than in its next factory. Authorization announcements have picked up through 2026, concentrated in technology, financials, and the parts of healthcare that generate more cash than ideas. The political noise around buybacks has not disappeared. It has been priced in as a cost of doing capital allocation in public.</p>
